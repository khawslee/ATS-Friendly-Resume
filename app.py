...
if page == "Rephase to Google XYZ":
    rephase_to_google_xyz_page(api_key, model_name)
elif page == "Core competencies extractor":
    core_competencies_extractor_page(api_key, model_name, job_description)
elif page == "Bullet Point Rephaser":
    bullet_point_rephaser_page(api_key, model_name, job_title, job_description)
elif page == "ATS Friendly Resume Generator":
    ats_friendly_resume_generator_page(api_key, model_name, uploaded_file, job_description)
elif page == "Generate Cover Letter":
    generate_cover_letter_page(api_key, model_name, uploaded_file, job_title, job_description)
elif page == "Introduction Generator":
    introduction_generator_page(api_key, model_name, uploaded_file, job_title, job_description)
elif page == "Tell me about yourself":
    tell_me_about_yourself_page(api_key, model_name, uploaded_file, job_title, job_description)
elif page == "Top Interview Questions":
    top_interview_questions_page(api_key, model_name, job_description, uploaded_file, page)
elif page == "Common Interview Questions":
    common_interview_questions_page(api_key, model_name, job_description, uploaded_file)
